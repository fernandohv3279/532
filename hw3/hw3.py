...
C_values = np.linspace(0.1, 12.1, 100)
best_C, best_acc = None, -1

for C in C_values:
    w, t = solve_svm(X_train, y_train, C)
    preds = np.sign(X_val @ w + t)
...
w, t = solve_svm(X_train, y_train, best_C)
test_acc = np.mean(np.sign(X_test @ w + t) == y_test)
